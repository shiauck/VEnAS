...
for(my $i = 0; $i <= $#as_type; $i++) {
   ...
   while(my $line = <FILE>) {
      ...
      if($line_arr[153] ne 'NA' && $line_arr[153] < 0.05) {
         ...
         $sig_homologene_h->{$line_arr[0]}->{$line_arr[1]}->{$line_arr[2]} = 1 if($line_arr[154] == 1);
         $sig_homolo_enh_h->{$line_arr[0]}->{$line_arr[1]}->{$line_arr[2]} = 1 if($line_arr[155] == 1);
         $sig_homo_h->{$line_arr[0]}->{$line_arr[1]}->{$line_arr[2]} = 1 if($line_arr[154] == 1 && $line_arr[155] == 1);
      }
   }
   close(FILE);
