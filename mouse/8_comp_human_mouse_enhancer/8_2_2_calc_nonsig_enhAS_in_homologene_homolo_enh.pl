...
for(my $i = 0; $i <= $#as_type; $i++) {
   ...
   while(my $line = <FILE>) {
      ...
      if(!defined $sig_all_h->{$line_arr[0]}) {
         $nonsig_all_h->{$line_arr[0]} = 1;
         $nonsig_homologene_h->{$line_arr[0]} = 1 if($line_arr[154] == 1);
         $nonsig_homolo_enh_h->{$line_arr[0]} = 1 if($line_arr[155] == 1);
         $nonsig_homo_h->{$line_arr[0]} = 1 if($line_arr[154] == 1 && $line_arr[155] == 1);
      }
   }
   close(FILE);
